...
<div class="container" style="max-width: 900px;">
    ...
    <div class="mb-4">
        ...
        <h1 class="fw-bold text-dark mt-2 mb-1">Edit Kandidat: {{ $candidate->full_name }}</h1>
        <p class="text-muted">Perbarui data profil pelamar</p>
    </div>
    ...
    <form action="{{ route('admin.candidates.update', $candidate->id) }}" method="POST">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="full_name" value="{{ old('full_name', $candidate->full_name) }}" placeholder="Nama lengkap kandidat" class="form-control" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Status Ketersediaan <span class="text-danger">*</span></label>
                        <select name="status" class="form-select" required>
                            <option value="tersedia" {{ old('status', $candidate->status) === 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                            <option value="disalurkan" {{ old('status', $candidate->status) === 'disalurkan' ? 'selected' : '' }}>Disalurkan</option>
                            <option value="tidak_aktif" {{ old('status', $candidate->status) === 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">No. Telepon / WhatsApp</label>
                        <input type="text" name="phone" value="{{ old('phone', $candidate->phone) }}" placeholder="Contoh: 0812xxxxxxxx" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Email</label>
                        <input type="email" name="email" value="{{ old('email', $candidate->email) }}" placeholder="nama.kandidat@domain.com" class="form-control">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Tanggal Lahir</label>
                        <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $candidate->date_of_birth ? $candidate->date_of_birth->format('Y-m-d') : '') }}" class="form-control">
                    </div>
                    <div class="col-md-6">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="headline" value="{{ old('headline', $candidate->headline) }}" placeholder="Contoh: Senior Web Developer / Administrasi Perkantoran" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Lokasi Tinggal Sekarang</label>
                        <input type="text" name="location" value="{{ old('location', $candidate->location) }}" placeholder="Contoh: Jakarta Selatan, Surabaya" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Gaji yang Diharapkan (Rp)</label>
                        <input type="text" name="expected_salary" value="{{ old('expected_salary', $candidate->expected_salary) }}" placeholder="Contoh: 5.000.000 / Nego" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Ketersediaan Waktu Kerja</label>
                        <input type="text" name="availability" value="{{ old('availability', $candidate->availability) }}" placeholder="Contoh: Segera / Pemberitahuan 1 Bulan" class="form-control">
                    </div>

                    <div class="col-12">
                        <label class="form-label small fw-semibold text-muted">Tentang / Deskripsi Diri</label>
                        <textarea name="about" rows="3" placeholder="Ceritakan ringkasan keahlian dan profil kandidat..." class="form-control">{{ old('about', $candidate->about) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    @foreach($skills as $skill)
                        <div class="col-md-4 col-sm-6">
                            <div class="form-check p-2 border rounded shadow-sm bg-white d-flex align-items-center">
                                <input class="form-check-input ms-1 me-2" type="checkbox" name="skills[]" value="{{ $skill->id }}" id="skill-{{ $skill->id }}" {{ in_array($skill->id, old('skills', $candidateSkills)) ? 'checked' : '' }}>
            ...
            <div class="card-body">
                <div id="experience-list">
                    @foreach($candidate->experiences as $exp)
                        <div class="border-bottom pb-3 mb-3 experience-row">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Posisi / Jabatan</label>
                                    <input type="text" name="exp_position[]" value="{{ $exp->position }}" placeholder="Contoh: Staf Admin" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Nama Perusahaan</label>
                                    <input type="text" name="exp_company[]" value="{{ $exp->company }}" placeholder="Contoh: PT. Abadi" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Tanggal Mulai</label>
                                    <input type="date" name="exp_start_date[]" value="{{ $exp->start_date }}" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Tanggal Selesai (Kosongkan jika masih bekerja)</label>
                                    <input type="date" name="exp_end_date[]" value="{{ $exp->end_date }}" class="form-control form-control-sm">
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label small fw-semibold text-muted">Deskripsi Tugas</label>
                                    <textarea name="exp_description[]" rows="2" placeholder="Tuliskan detail pekerjaan..." class="form-control form-control-sm">{{ $exp->description }}</textarea>
                                </div>
            ...
            <div class="card-body">
                <div id="education-list">
                    @foreach($candidate->educations as $edu)
                        <div class="border-bottom pb-3 mb-3 education-row">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Institusi Pendidikan</label>
                                    <input type="text" name="edu_institution[]" value="{{ $edu->institution }}" placeholder="Contoh: Universitas Indonesia" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Gelar / Bidang Studi</label>
                                    <input type="text" name="edu_degree[]" value="{{ $edu->degree }}" placeholder="Contoh: S1 Akuntansi / SMA IPA" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Tahun Masuk</label>
                                    <input type="number" name="edu_start_year[]" value="{{ $edu->start_year }}" placeholder="Contoh: 2018" class="form-control form-control-sm" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-semibold text-muted">Tahun Lulus (Kosongkan jika belum lulus)</label>
                                    <input type="number" name="edu_end_year[]" value="{{ $edu->end_year }}" placeholder="Contoh: 2022" class="form-control form-control-sm">
                                </div>
                                <div class="col-md-12 text-end">
                                    <button type="button" class="btn btn-sm btn-outline-danger py-1 px-3 rounded-pill remove-row-btn">Hapus Baris</button>
                                </div>
        ...
        <div class="mb-5 d-flex gap-2">
            <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold py-2.5">Perbarui Data Kandidat</button>
