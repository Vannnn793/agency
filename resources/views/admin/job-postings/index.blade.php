...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="fw-bold text-dark mb-1">Daftar Lowongan Pekerjaan</h1>
            <p class="text-muted mb-0">Kelola semua lowongan kerja yang diunggah oleh perusahaan mitra</p>
        </div>
        <a href="{{ route('admin.job-postings.create') }}" class="btn btn-primary rounded-pill px-4 fw-bold">➕ Buat Lowongan</a>
    </div>
    ...
    <div class="card shadow-sm border-0 mb-4 bg-white">
        <div class="card-body">
            <form action="{{ route('admin.job-postings.index') }}" method="GET" class="row g-3">
                <div class="col-md-4">
                    <label class="form-label small fw-semibold text-muted">Perusahaan</label>
                ...
                <div class="col-md-4 d-flex align-items-end">
                    <button class="btn btn-outline-secondary w-100 rounded-pill fw-bold">Filter Lowongan</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            <th class="ps-4">Lowongan Pekerjaan</th>
                            <th>Perusahaan</th>
                            <th>Lokasi</th>
                            <th>Rentang Gaji</th>
                            <th>Status</th>
                            <th>Skills Diminta</th>
                    ...
                    <tbody>
                        @forelse($jobPostings as $job)
                            <tr>
                                ...
                                <td>
                                    <div class="fw-semibold text-secondary">{{ $job->company->name }}</div>
                                </td>
                                ...
                                <td class="text-end pe-4">
                                    <a href="{{ route('admin.job-postings.show', $job->id) }}" class="btn btn-sm btn-outline-primary px-3 rounded-pill fw-bold">Detail / Match</a>
                                    <a href="{{ route('admin.job-postings.edit', $job->id) }}" class="btn btn-sm btn-outline-secondary px-3 rounded-pill fw-bold ms-1">Edit</a>
                                    <form action="{{ route('admin.job-postings.destroy', $job->id) }}" method="POST" class="d-inline ms-1">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    Tidak ada data lowongan pekerjaan yang cocok.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
