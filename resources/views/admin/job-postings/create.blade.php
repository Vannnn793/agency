...
<div class="container" style="max-width: 800px;">
    ...
    <div class="mb-4">
        ...
        <h1 class="fw-bold text-dark mt-2 mb-1">Buat Lowongan Pekerjaan Baru</h1>
        <p class="text-muted">Masukkan rincian kebutuhan lowongan pekerjaan</p>
    </div>
    ...
    <form action="{{ route('admin.job-postings.store') }}" method="POST">
        ...
        <div class="card shadow-sm border-0 mb-4 bg-white">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Pilih Perusahaan <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: Flutter Developer / Staf Admin" class="form-control" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Lokasi Penempatan</label>
                        <input type="text" name="location" value="{{ old('location') }}" placeholder="Contoh: Jakarta Pusat, Remote" class="form-control">
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="filled" {{ old('status') === 'filled' ? 'selected' : '' }}>Terisi (Filled)</option>
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Minimum Gaji (Rp)</label>
                        <input type="number" name="salary_min" value="{{ old('salary_min') }}" placeholder="Contoh: 4000000" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Maksimum Gaji (Rp)</label>
                        <input type="number" name="salary_max" value="{{ old('salary_max') }}" placeholder="Contoh: 6000000" class="form-control">
                    </div>

                    <div class="col-12">
                        <label class="form-label small fw-semibold text-muted">Deskripsi Pekerjaan & Kualifikasi</label>
                        <textarea name="description" rows="5" placeholder="Tuliskan deskripsi pekerjaan, tanggung jawab, dan kualifikasi yang diinginkan..." class="form-control">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="education_requirement" class="form-select">
                            ...
                            <option value="SMA" {{ old('education_requirement') === 'SMA' ? 'selected' : '' }}>SMA / Sederajat</option>
                            <option value="SMK" {{ old('education_requirement') === 'SMK' ? 'selected' : '' }}>SMK</option>
                            <option value="D3" {{ old('education_requirement') === 'D3' ? 'selected' : '' }}>DIPLOMA 3 (D3)</option>
                            <option value="S1" {{ old('education_requirement') === 'S1' ? 'selected' : '' }}>SARJANA 1 (S1)</option>
                            <option value="S2" {{ old('education_requirement') === 'S2' ? 'selected' : '' }}>PASCA SARJANA (S2)</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold text-muted">Minimal Pengalaman Kerja (Tahun)</label>
                        <input type="number" name="experience_years" value="{{ old('experience_years') }}" placeholder="Contoh: 2" class="form-control">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mb-5 d-flex gap-2">
            <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold py-2.5">Terbitkan Lowongan Kerja</button>
            <a href="{{ route('admin.job-postings.index') }}" class="btn btn-outline-secondary rounded-pill px-4 fw-bold py-2.5">Batal</a>
        </div>
    </form>
</div>
@endsection
